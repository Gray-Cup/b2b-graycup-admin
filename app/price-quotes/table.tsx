...
export function PriceQuotesTable() {
  ...
  const toggleSelectAll = () => {
    ...
    else setSelectedIds(new Set(filteredData.map((row) => row.id)))
  }
  ...
  const toggleResolved = async (id: string, currentValue: boolean) => {
    try {
      await updateSubmission('price_quotes', id, !currentValue, resolvedParam)
  ...
  const toggleVaulted = async (id: string, currentValue: boolean) => {
    try {
      await vaultSubmission('price_quotes', id, !currentValue, resolvedParam)
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteSubmission('price_quotes', id, resolvedParam)
  ...
  return (
    <>
      <PageHeader
        title="Price Quotes"
        description="Price quote requests from bulkgreencoffee.com"
        action={<DownloadButton tableName="price_quotes" title="Price Quotes" />}
      />
      ...
      {isLoading ? (
        ...
        <div className="p-8 text-center bg-ui-bg-base rounded-lg border border-ui-border-base">
          <Text className="text-ui-fg-subtle">No price quotes found</Text>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredData.map((item) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <Text className="font-medium text-ui-fg-base truncate">{item.company_name || 'No company'}</Text>
                    <Text className="text-sm text-ui-fg-subtle">{item.contact_name}</Text>
                  </div>
                </div>
              ...
              {item.notes && (
                <div>
                  <Text className="text-xs text-ui-fg-muted uppercase tracking-wide mb-1">Notes</Text>
                  <Text className="text-sm text-ui-fg-subtle line-clamp-2">{item.notes}</Text>
                </div>
              )}
